...
export class BookResolverService {
  ...
  async resolveByIsbn(isbn) {
    ...
    if (cached) {
      return {
        status: 200,
        body: { ...toPublicBook(cached), cache: cached.cache },
      };
    }

    const serpResultsPromise = this.safeSource('serp', () =>
      fetchSearchResults(normalizedIsbn, {
        timeoutMs: this.timeoutMs,
      })
    );

    const livelibPromise = this.safeSource('livelib', async () => {
      const direct = await fetchLiveLibByIsbn(normalizedIsbn, {
        timeoutMs: this.timeoutMs,
      });

      if (direct?.length) return direct;

      const serpResults = await serpResultsPromise;
      return fetchLiveLibFromSearchResults(serpResults, {
        timeoutMs: this.timeoutMs,
      });
    });

    const [google, openLibrary, serpResults, livelib] = await Promise.all([
      this.safeSource('google_books', () =>
        fetchGoogleBookByIsbn(normalizedIsbn, {
          timeoutMs: this.timeoutMs,
        })
      ),
      this.safeSource('open_library', () =>
        fetchOpenLibraryBookByIsbn(normalizedIsbn, {
          timeoutMs: this.timeoutMs,
        })
      ),
      serpResultsPromise,
    ...
    const best =
      normalized.find(
        (book) =>
          normalizeIsbn(book.isbn) === normalizedIsbn
      ) || normalized[0];

    if (!best) {
      return {
        status: 404,
        body: { error: 'Book not found', isbn: normalizedIsbn },
      };
    }
  ...
  async search(query) {
    ...
    const livelibPromise = serpResultsPromise.then((serpResults) =>
      this.safeSource('livelib', () =>
        fetchLiveLibFromSearchResults(serpResults, {
          timeoutMs: this.timeoutMs,
        })
      )
    );
    ...
    const [google, openLibrary, serpResults, livelib] = await Promise.all([
      this.safeSource('google_books', () =>
        searchGoogleBooks(q, { timeoutMs: this.timeoutMs })
      ),
      this.safeSource('open_library', () =>
        searchOpenLibraryBooks(q, { timeoutMs: this.timeoutMs })
      ),
      serpResultsPromise,
      livelibPromise,
    ]);
